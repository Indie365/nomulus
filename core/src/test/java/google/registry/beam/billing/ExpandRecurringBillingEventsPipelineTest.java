...
public class ExpandRecurringBillingEventsPipelineTest {
  ...
  private DateTime startTime = DateTime.parse("2022-02-01TZ");

  private DateTime endTime = DateTime.parse("2022-02-02TZ");

  private Cursor cursor = Cursor.createGlobal(RECURRING_BILLING, startTime);

  private Domain domain;

  private Recurring recurring;

  private ExpandRecurringBillingEventsPipeline expandRecurringBillingEventsPipeline;

  private TestOptions options = PipelineOptionsFactory.create().as(TestOptions.class);
  ...
  void testFailure_endTimeBeforeStartTime() {
    ...
    IllegalArgumentException thrown =
        assertThrows(IllegalArgumentException.class, () -> runPipeline());
    assertThat(thrown)
        .hasMessageThat()
  ...
  void testFailure_expandSingleEvent_cursorNotAtStartTime() {
    ...
    PipelineExecutionException thrown =
        assertThrows(PipelineExecutionException.class, () -> runPipeline());
  ...
  void testSuccess_noExpansion_oneTimeAlreadyExists() {
    ...
    // Assert about BillingEvents. No expansion happend, so last recurrence expansion time is
    // unchanged.
  ...
  // We control the number of threads used in the pipeline to test if the batching hehavior works
  // properly. When two threads are used, the two recurrings are processed in different workers and
  // should be processed in parallel. In tests the isolation level is set to serializable, which
  // should cause one of the concurrent write to fail and retry until the other one finishes.
  @ParameterizedTest
  ...
  void testSuccess_expandMultipleEvents_multipleEventTime() {
    ...
    ImmutableList<DomainHistory> histories =
        loadHistoryObjectsForResource(domain.createVKey(), DomainHistory.class).stream()
            ...
            .sorted(
                (h1, h2) ->
                    h1.getDomainTransactionRecords().stream()
                        .findFirst()
                        .get()
                        .getReportingTime()
                        .compareTo(
                            h2.getDomainTransactionRecords().stream()
                                .findFirst()
                                .get()
                                .getReportingTime()))
  ...
  private void runPipeline() {
    expandRecurringBillingEventsPipeline = new ExpandRecurringBillingEventsPipeline(options);

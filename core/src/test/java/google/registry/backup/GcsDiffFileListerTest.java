...
public class GcsDiffFileListerTest {
  ...
  private final GcsUtils gcsUtils = new GcsUtils(LocalStorageHelper.getOptions().getService());
  ...
  void testList_patchesHoles() throws Exception {
    ...
    diffLister.gcsUtils = mockGcsUtils;
    when(mockGcsUtils.listFolderObjects(anyString(), anyString()))
        .thenReturn(
            ImmutableList.of(DIFF_FILE_PREFIX + now, DIFF_FILE_PREFIX + now.minusMinutes(4)));

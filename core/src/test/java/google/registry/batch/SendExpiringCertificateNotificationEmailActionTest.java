...
class SendExpiringCertificateNotificationEmailActionTest {
  ...
  void beforeEach() throws Exception {
    ...
    registrar = createRegistrar("sampleRegistrar", null, null);
  }
  ...
  private Registrar createRegistrar(
      ...
      throws Exception {
    // set up only required fields with sample test data
        ...
        new Registrar.Builder()
            ...
            .setInternationalizedAddress(
                new RegistrarAddress.Builder()
                    .setStreet(ImmutableList.of("street"))
                    .setCity("city")
                    .setState("state")
                    .setZip("99999")
                    .setCountryCode("US")
  ...
  void getRegistrarsWithExpiringCertificates_returnsPartOfRegistrars() throws Exception {
    ...
    for (int i = 1; i <= numOfRegistrarsWithExpiringCertificates; i++) {
      Registrar reg = createRegistrar("name" + i, expiringCertificate, null);
      persistResource(reg);
    }
    for (int i = numOfRegistrarsWithExpiringCertificates; i <= numOfRegistrars; i++) {
      Registrar reg = createRegistrar("name" + i, certificate, null);
      persistResource(reg);
    }
  ...
  void getRegistrarsWithExpiringCertificates_returnsPartOfRegistrars_failOverCertificateBranch()
    ...
    int numOfRegistrarsWithExpiringCertificates = 2;
    for (int i = 1; i <= numOfRegistrarsWithExpiringCertificates; i++) {
      Registrar reg = createRegistrar("name" + i, null, expiringCertificate);
      persistResource(reg);
    }
    for (int i = numOfRegistrarsWithExpiringCertificates; i <= numOfRegistrars; i++) {
      Registrar reg = createRegistrar("name" + i, null, certificate);
      persistResource(reg);
    }

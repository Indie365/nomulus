...
public class ExpandRecurringBillingEventsActionTest extends BeamActionTestBase {
  ...
  void testSuccess_provideEndTime() throws Exception {
    action.endTimeParam = Optional.of(DateTime.parse("2020-02-01T12:00:00Z"));
    expectedParameters.put("endTime", "2020-02-01T12:00:00Z");
    action.run();
    assertThat(response.getStatus()).isEqualTo(200);
    assertThat(response.getPayload())
        .isEqualTo("Launched recurring billing event expansion pipeline: jobid");
  ...
  void testSuccess_provideStartTime() throws Exception {
    action.startTimeParam = Optional.of(DateTime.parse("2020-01-01T12:00:00Z"));
    expectedParameters.put("startTime", "2020-01-01T12:00:00Z");
    action.run();
    assertThat(response.getStatus()).isEqualTo(200);
    assertThat(response.getPayload())
        .isEqualTo("Launched recurring billing event expansion pipeline: jobid");

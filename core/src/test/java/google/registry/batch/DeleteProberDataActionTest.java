...
import static google.registry.testing.DatabaseHelper.existsInDatabase;
...
class DeleteProberDataActionTest extends MapreduceTestCase<DeleteProberDataAction> {
  ...
  void testSuccess_doesntDeleteNicDomainForProbers() throws Exception {
    ...
    runMapreduce();
    assertDeleted(ibEntities);
    assertNotDeleted(ImmutableSet.of(nic));
    if (tm().isOfy()) {
      assertNotDeleted(ImmutableSet.of(fkiNic));
    }
  }
  ...
  void test_domainWithSubordinateHosts_isSkipped() throws Exception {
    ...
    runMapreduce();
    assertThat(existsInDatabase(domainWithSubord)).isTrue();
    assertThat(existsInDatabase(nakedDomain)).isFalse();
  }
  ...
  private static void assertNotDeleted(Iterable<ImmutableObject> entities) {
    for (ImmutableObject entity : entities) {
      assertWithMessage("Expected entity %s to exist in the DB but it was absent", entity)
          .that(existsInDatabase(entity))
          .isTrue();
    }
  }
  ...
  private static void assertDeleted(Iterable<ImmutableObject> entities) {
    for (ImmutableObject entity : entities) {
      assertWithMessage("Expected entity %s to not exist in the DB but it was present", entity)
          .that(existsInDatabase(entity))
          .isFalse();
    }
  }

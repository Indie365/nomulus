...
public class ClaimsListDaoTest {
  ...
  void testEntryCaching_savesAndUpdates() {
    ...
    assertThat(fromDatabase.CLAIM_KEY_CACHE.getIfPresent("label1")).isNull();
    Truth8.assertThat(fromDatabase.getClaimKey("label1")).hasValue("key1");
    // After retrieval, the key exists
    Truth8.assertThat(fromDatabase.CLAIM_KEY_CACHE.getIfPresent("label1")).hasValue("key1");
    assertThat(fromDatabase.CLAIM_KEY_CACHE.getIfPresent("label2")).isNull();
    ...
    assertThat(fromDatabase.CLAIM_KEY_CACHE.getIfPresent("nonexistent")).isNull();
    Truth8.assertThat(fromDatabase.getClaimKey("nonexistent")).isEmpty();
    Truth8.assertThat(fromDatabase.CLAIM_KEY_CACHE.getIfPresent("nonexistent")).isEmpty();
  }

...
public class CursorTest extends EntityTestCase {
  ...
  void testSuccess_persistScopedCursor() {
    ...
    tm().transact(
            () -> {
              assertThat(tm().loadByKeyIfPresent(Cursor.createVKey(BRDA, "tld")).isPresent())
                  .isFalse();
  ...
  void testFailure_VKeyWrongScope() {
    assertThrows(
        ...
        () -> Cursor.createVKey(RDE_UPLOAD, "GLOBAL"),
        "Scope GLOBAL does not match cursor type RDE_UPLOAD");
    assertThrows(RegistryNotFoundException.class, () -> Cursor.createVKey(RDE_UPLOAD, "tld"));
    assertThrows(
        IllegalArgumentException.class,
        () -> Cursor.createVKey(RECURRING_BILLING, "tld"),
        "Scope tld does not match cursor type RECURRING_BILLING");
    assertThrows(NullPointerException.class, () -> Cursor.createVKey(RECURRING_BILLING, null));
  }

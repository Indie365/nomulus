...
@DualDatabaseTest // only stored in ofy, but use both to make sure that it works in both worlds
public class DatabaseMigrationStateWrapperTest {
  ...
  void testEmpty_canChangeToDatastorePrimary() {
    ofyTm().transact(() -> DatabaseMigrationStateWrapper.set(DATASTORE_PRIMARY));
  ...
  private static void runValidTransition(MigrationState from, MigrationState to) {
    setStateForced(from);
    ofyTm().transact(() -> DatabaseMigrationStateWrapper.set(to));

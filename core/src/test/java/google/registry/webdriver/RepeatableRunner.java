  ...
  private static class RepeatableStatement extends Statement {
    ...
    public void evaluate() throws Throwable {
      ...
      for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        // attemptNumber would be null if there is any exception happens
        // during the instantiation of test class object(including test
        // rules). However, those exceptions would not be actually thrown
        // until statement.evaluate() is invoked. So, we should just skip
        // the setter and let the statement be evaluated. Then we should
        // be able to find the stack trace of root cause in the test reports.

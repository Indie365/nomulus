...
public class RdeReportActionTest {
  ...
  private FakeUrlConnectionService urlConnectionService;

  private RdeReportAction createAction() throws IOException {
  ...
  void beforeEach() throws Exception {
    ...
    urlConnectionService = new FakeUrlConnectionService();
  }
  ...
  void testRunWithLock() throws Exception {
    when(urlConnectionService.getMockConnection().getResponseCode()).thenReturn(SC_OK);
    when(urlConnectionService.getMockConnection().getInputStream())
        .thenReturn(IIRDEA_GOOD_XML.openStream());
    ...
    verify(urlConnectionService.getMockConnection()).setRequestMethod("PUT");
    assertThat(urlConnectionService.getMockConnection().getURL().getProtocol()).isEqualTo("https");
    assertThat(urlConnectionService.getMockConnection().getURL().getPath())
        .endsWith("/test/20101017001");
    verify(urlConnectionService.getMockConnection())
        .setRequestProperty("Content-Type", "text/xml; charset=utf-8");
    verify(urlConnectionService.getMockConnection())
        .setRequestProperty("Authorization", "Basic dGVzdF9yeTpmb28=");

    // Verify the payload XML was the same as what's in testdata/report.xml.
    XjcRdeReportReport report =
        parseReport(urlConnectionService.getConnectionOutputStream().toByteArray());
  ...
  void testRunWithLock_badRequest_throws500WithErrorInfo() throws Exception {
    when(urlConnectionService.getMockConnection().getResponseCode()).thenReturn(SC_BAD_REQUEST);
    when(urlConnectionService.getMockConnection().getInputStream())
        .thenReturn(IIRDEA_BAD_XML.openStream());
  ...
  void testRunWithLock_fetchFailed_throwsRuntimeException() throws Exception {
    ...
    when(urlConnectionService.getMockConnection().getResponseCode())
        .thenThrow(new ExpectedThrownException());
    assertThrows(

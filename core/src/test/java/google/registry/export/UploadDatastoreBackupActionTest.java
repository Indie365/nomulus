...
public class UploadDatastoreBackupActionTest {
  ...
  Task getChainedTask(Task task) {
    try {
      return (Task)
          new ObjectInputStream(
                  new ByteArrayInputStream(task.getAppEngineHttpRequest().getBody().toByteArray()))
              .readObject();
    } catch (ClassNotFoundException | IOException e) {
      return cloudTasksUtils.createPostTask("", "", ImmutableMultimap.of());
    }
  }

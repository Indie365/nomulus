...
class NordnVerifyActionTest {
  ...
  void testSuccess_sendHttpRequest_urlIsCorrect() throws Exception {
    action.run();
    assertThat(httpsURLConnection.getURL()).isEqualTo(new URL("http://127.0.0.1/blobio"));
  }
  ...
  void failureVerifyUnauthorized() throws Exception {
    when(httpsURLConnection.getResponseCode()).thenReturn(SC_UNAUTHORIZED);
    assertThrows(Exception.class, action::run);
  }

  @Test
  void failureVerifyNotReady() throws Exception {
    when(httpsURLConnection.getResponseCode()).thenReturn(SC_NO_CONTENT);

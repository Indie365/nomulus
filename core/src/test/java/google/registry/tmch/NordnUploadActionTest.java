...
class NordnUploadActionTest {
  ...
  void beforeEach() throws Exception {
    ...
    when(urlConnectionService.getMockConnection().getResponseCode()).thenReturn(SC_ACCEPTED);
    when(urlConnectionService.getMockConnection().getHeaderField(LOCATION))
        .thenReturn("http://trololol");
  ...
  void testRun_claimsMode_appendsTldAndClaimsToRequestUrl() throws Exception {
    ...
    action.run();
    assertThat(urlConnectionService.getMockConnection().getURL())
  ...
  void testRun_usesMultipartContentType() throws Exception {
    ...
    verify(urlConnectionService.getMockConnection()).setRequestMethod("POST");
  }
  ...
  void testRun_noPassword_doesntSendAuthorizationHeader() {
    ...
    persistClaimsModeDomain();
    action.run();
    verify(urlConnectionService.getMockConnection(), times(0))
        .setRequestProperty(eq(AUTHORIZATION), anyString());
  }
  ...
  void testRun_claimsMode_payloadMatchesClaimsCsv() {
    persistClaimsModeDomain();
    action.run();
    assertThat(new String(urlConnectionService.getConnectionOutputStream().toByteArray(), UTF_8))
        .contains(CLAIMS_CSV);
  }
  ...
  void testFetchFailure() throws Exception {
    ...
    when(urlConnectionService.getMockConnection().getResponseCode())
        .thenReturn(SC_INTERNAL_SERVER_ERROR);

...
public class CloudTasksHelper {
  ...
  public static void assertTasksEnqueuedWithProperty(
      ...
      String queueName,
      Function<Task, String> propertyGetter,
      String... expectedTaskProperties) {
    // Ordering is irrelevant but duplicates should be considered independently.
    Truth8.assertThat(cloudTasksUtils.getTestTasksFor(queueName).stream().map(propertyGetter))
  ...
  public static void assertTasksEnqueued(
      CloudTasksUtils cloudTasksUtils, String queueName, String... expectedTaskNames) {
    Function<Task, String> nameGetter = Task::getName;
    assertTasksEnqueuedWithProperty(cloudTasksUtils, queueName, nameGetter, expectedTaskNames);
  }
   ...
   */
  public static void assertTasksEnqueued(
      CloudTasksUtils cloudTasksUtils, String queueName, TaskMatcher... taskMatchers) {
    assertTasksEnqueued(cloudTasksUtils, queueName, asList(taskMatchers));
  }

  /** Ensures that the named queue contains no tasks. */
  public static void assertNoTasksEnqueued(CloudTasksUtils cloudTasksUtils, String... queueNames) {
    for (String queueName : queueNames) {
      assertThat(cloudTasksUtils.getTestTasksFor(queueName)).isEmpty();
    }
  }
   ...
   */
  public static void assertTasksEnqueued(
      CloudTasksUtils cloudTasksUtils, String queueName, Collection<TaskMatcher> taskMatchers) {
    List<Task> tasks = cloudTasksUtils.getTestTasksFor(queueName);
  ...
  private static class MatchableTask extends ImmutableObject {
    ...
    MatchableTask(Task task) {
      ...
      if (hasFormDataContentType && method.equals(HttpMethod.POST)) {

...
public class RegistrarPoc extends ImmutableObject implements Jsonifiable, UnsafeSerializable {
   ...
   * creating a {@link RegistrarPoc}. We switched to using the login email directly as each registrar is
   * assigned a unique email address that is immutable (to them at least), so it is as good as an
   * identifier as the ID itself, and it allows us to get rid of the datastore dependency.
   *

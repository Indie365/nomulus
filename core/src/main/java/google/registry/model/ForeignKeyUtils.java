...
public final class ForeignKeyUtils {
   ...
   * recently created foreign key index was deleted before time "now".
   *
            ...
            jpaTmToUse
                .query(
                    "SELECT %fkProperty%, repoId, deletionTime FROM %entity WHERE (%fkProperty%,"
                        + " deletionTime) IN (SELECT %fkProperty%, MAX(deletionTime) FROM %entity%"
                        + " WHERE %fkProperty% in (:fks) GROUP BY %fkProperty)"
  ...
  static final CacheLoader<VKey<? extends EppResource>, MostRecentResource> CACHE_LOADER =
      new CacheLoader<VKey<? extends EppResource>, MostRecentResource>() {

        @Override
        @Nullable
        public MostRecentResource load(VKey<? extends EppResource> key) {
          return loadAll(ImmutableList.of(key)).get(key);
        }

        @Override
        public Map<VKey<? extends EppResource>, MostRecentResource> loadAll(
          ...
          ImmutableList<String> foreignKeys =
              Streams.stream(keys).map(key -> (String) key.getSqlKey()).collect(toImmutableList());
          return ForeignKeyUtils.load(clazz, foreignKeys, true).entrySet().stream()
              .collect(toImmutableMap(e -> VKey.createSql(clazz, e.getKey()), Entry::getValue));
        }
      };
   ...
   */
  @NonFinalForTesting
  private static LoadingCache<VKey<? extends EppResource>, MostRecentResource> foreignKeyCache =
      createForeignKeyMapCache(getEppResourceCachingDuration());

  private static LoadingCache<VKey<? extends EppResource>, MostRecentResource>
  ...
  /**
   * Load a list of {@link ForeignKeyUtils} instances by class and repoId strings that are active at
   * or after the specified moment in time, using the cache if enabled.
   *
   * <p>The returned map will omit any keys for which the {@link ForeignKeyUtils} doesn't exist or
  ...
  public static <E extends EppResource> ImmutableMap<String, VKey<E>> loadCached(
    ...
    return foreignKeyCache
        ...
        .entrySet()
        .stream()
        .filter(e -> e.getValue() != null && now.isBefore(e.getValue().deletionTime()))
        .collect(
            toImmutableMap(
                e -> (String) e.getKey().getSqlKey(),
                e -> VKey.createSql(clazz, e.getValue().repoId())));
  }

...
public final class ForeignKeyUtils {
   ...
   *
   * <p>Note that in production, the {@code deletionTime} should monotonically increase as one
   * cannot create a domain/host/contact with the same foreign key without soft deleting the
   * existing resource first. However, in test, there's no such guarantee and one must make sure
   * that no two resources with the same foreign key exist with the same max {@code deleteTime},
   * usually {@code END_OF_TIME}, least this method throws an error due to duplicate keys.
   */

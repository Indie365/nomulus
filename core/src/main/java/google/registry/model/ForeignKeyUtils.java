...
public final class ForeignKeyUtils {
  ...
  static final CacheLoader<VKey<? extends EppResource>, Optional<MostRecentResource>> CACHE_LOADER =
      new CacheLoader<VKey<? extends EppResource>, Optional<MostRecentResource>>() {
        ...
        public Map<VKey<? extends EppResource>, Optional<MostRecentResource>> loadAll(
          ...
          // The above map only contains keys that exist in the database, so we re-add the missing
          // ones with Optional.empty() values for caching.
          return Maps.asMap(

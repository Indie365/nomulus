...
public class PremiumListDualDao {
  ...
  public static Optional<Money> getPremiumPrice(String label, Registry registry) {
    ...
    try {
      ...
      if (tm().isOfy()) {
        secondaryResult = PremiumListSqlDao.getPremiumPrice(premiumList, label);
      } else {
        secondaryResult =
            PremiumListDatastoreDao.getPremiumPrice(premiumList, label, registry.getTldStr());
      }
      if (!primaryResult.equals(secondaryResult)) {
        logger.atWarning().log(
            "Unequal prices for domain %s.%s from primary DB (%s) and secondary DB (%s).",
  ...
  public static PremiumList save(String name, List<String> inputData) {
    ...
    if (tm().isOfy()) {
      ...
      runAndLogExceptions(() -> PremiumListSqlDao.save(name, inputData), "save");
    } else {
      result = PremiumListSqlDao.save(name, inputData);
      runAndLogExceptions(() -> PremiumListDatastoreDao.save(name, inputData), "save");
    }
  ...
  public static void delete(PremiumList premiumList) {
    if (tm().isOfy()) {
      ...
      runAndLogExceptions(() -> PremiumListSqlDao.delete(premiumList), "deletion");
    } else {
      PremiumListSqlDao.delete(premiumList);
      runAndLogExceptions(() -> PremiumListDatastoreDao.delete(premiumList), "deletion");
    }
  ...
  private static Optional<PremiumList> loadPremiumListUncached(String premiumListName) {
    if (tm().isOfy()) {
      return PremiumListDatastoreDao.getLatestRevisionUncached(premiumListName);
    } else {
      return PremiumListSqlDao.getLatestRevisionUncached(premiumListName);
    }
  }

  /** Method used for swallowing (but logging) exceptions when running against the secondary DB. */
  private static void runAndLogExceptions(Runnable runnable, String action) {
    try {
      runnable.run();
    } catch (Exception e) {
      logger.atSevere().withCause(e).log(
          "Error when performing action %s with secondary database.", action);
    }
  }

 ...
 *
 * <p>This class handles the mapping from PremiumList objects to the contents of those premium lists
 * in the Datastore world. Specifically, this deals with retrieving the most recent revision for a
 * given list and retrieving (or writing/deleting) all entries associated with that particular
 * revision. The {@link PremiumList} object itself, in the Datastore world, does not store the
...
public class PremiumListDatastoreDao {
  ...
  public static Optional<PremiumList> getLatestRevisionUncached(String name) {
   ...
   * premium.
   */
  public static Optional<Money> getPremiumPrice(PremiumList premiumList, String label, String tld) {
    DateTime startTime = DateTime.now(UTC);
    // If we're dealing with a list from SQL, reload from Datastore if necessary
    if (premiumList.getRevisionKey() == null) {
      Optional<PremiumList> fromDatastore = getLatestRevisionUncached(premiumList.getName());
    ...
    return ofyTm()
        .transactNew(
            () -> {
              ...
              assertOfyFieldsEqual(existing, oldPremiumList);
              PremiumList newList =
                  premiumList
                      .asBuilder()
  ...
  private static void assertOfyFieldsEqual(

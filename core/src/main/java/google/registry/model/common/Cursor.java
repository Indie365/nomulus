...
public class Cursor extends ImmutableObject implements NonReplicatedEntity {
  ...
  void onLoad() {
    ...
    Key<?> scopeKey = Key.valueOf(Splitter.on('_').splitToList(id).get(0));
    if (getCrossTldKey().equals(scopeKey)) {
      scope = GLOBAL;
    } else {
      scope = scopeKey.getName();
    }
  }
  ...
  void postLoad() {
    ...
    if (GLOBAL.equals(scope)) {
      scopeKey = getCrossTldKey();
    } else {
      scopeKey = Key.create(getCrossTldKey(), Registry.class, scope);
    }
  ...
  public static VKey<Cursor> createVKey(CursorType type, String scope) {
    Key<Cursor> key =
        GLOBAL.equals(scope) ? createGlobalKey(type) : createKey(type, Registry.get(scope));
  ...
  public static Key<Cursor> createGlobalKey(CursorType cursorType) {
    ...
    return Key.create(
        getCrossTldKey(), Cursor.class, generateId(cursorType, getCrossTldKey()));
  }
  ...
  private static Cursor create(
    ...
    instance.scope = getCrossTldKey().equals(scope) ? GLOBAL : scope.getName();
    return instance;
  }

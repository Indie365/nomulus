...
public class DatabaseMigrationStateSchedule extends CrossTldSingleton
  ...
  private static final Duration CACHE_INTERVAL = Duration.standardMinutes(5);
  ...
  private static final LoadingCache<
      ...
      CACHE =
          CacheBuilder.newBuilder()
              .expireAfterWrite(CACHE_INTERVAL.getStandardMinutes(), TimeUnit.MINUTES)
              .build(
                  new CacheLoader<
                      Class<DatabaseMigrationStateSchedule>,
                      TimedTransitionProperty<MigrationState, MigrationStateTransition>>() {
  ...
  /** Loads the currently-set migration schedule from the cache. */
  ...
  /** Loads the currently-set migration chedule from Datastore. */
  @VisibleForTesting
        ...
        .transact(
            () ->
                ofyTm()
                    ...
                    .orElse(DATASTORE_ONLY_TRANSITION_MAP));
  }

  // A new map can be valid by itself but imply a bad transition at the current time from old->new

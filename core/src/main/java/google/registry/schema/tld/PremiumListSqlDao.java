 ...
 *
 * <p>This class handles the mapping and retrieval of {@link PremiumEntry} objects that correspond
 * to the particular {@link PremiumList} object in SQL, and caching these entries so that future
 * lookups can be quicker.
 */
public class PremiumListSqlDao {
   ...
   */
  public static Optional<PremiumList> getLatestRevisionUncached(String premiumListName) {
    return jpaTm()
        .transact(
            () ->
                jpaTm()
   ...
   * premium.
   */
  public static Optional<Money> getPremiumPrice(PremiumList premiumList, String label) {
    Optional<PremiumList> maybeLoadedList = loadIfNecessary(premiumList);
  ...
  public static void delete(PremiumList premiumList) {
    jpaTm().transact(() -> loadIfNecessary(premiumList).ifPresent(jpaTm()::delete));
  }
  ...
  private static Optional<PremiumList> loadIfNecessary(PremiumList premiumList) {
    // If the premium list was loaded from ofy, all we have is the name, so load the full list
    if (premiumList.getCurrency() == null) {
      return getLatestRevisionUncached(premiumList.getName());
    }
    return Optional.of(premiumList);
  }

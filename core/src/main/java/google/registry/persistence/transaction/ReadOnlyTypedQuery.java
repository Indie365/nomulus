...
/** An implementation of {@link TypedQuery <T>} that throws exceptions on write actions. */
class ReadOnlyTypedQuery<T> implements TypedQuery<T> {

  private final TypedQuery<T> delegate;

  ReadOnlyTypedQuery(TypedQuery<T> delegate) {

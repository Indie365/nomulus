...
public final class TransactionManagerFactory {
  ...
  @NonFinalForTesting
  private static Supplier<JpaTransactionManager> readOnlyJpaTm =
      Suppliers.memoize(TransactionManagerFactory::createReadOnlyJpaTransactionManager);
  ...
  private static JpaTransactionManager createReadOnlyJpaTransactionManager() {
    if (isInAppEngine()) {
      return DaggerPersistenceComponent.create().readOnlyJpaTransactionManager();
    } else {
      return DummyJpaTransactionManager.create();
    }
  }
  ...
  public static JpaTransactionManager readOnlyJpaTm() {
    return readOnlyJpaTm.get();
  }

  /**
   * Returns a {@link TransactionManager} that, ideally, is read-only.
   *
   ...
   */
  public static TransactionManager readOnlyTm() {
    TransactionManager tm = tm();
    return tm.isOfy() ? tm : readOnlyJpaTm();
  }
  ...
  /** Sets the value of {@link #readOnlyJpaTm()} to the given {@link JpaTransactionManager}. */
  public static void setReadOnlyJpaTm(Supplier<JpaTransactionManager> readOnlyJpaTmSupplier) {
    checkNotNull(readOnlyJpaTmSupplier, "readOnlyJpaTmSupplier");
    checkState(
        RegistryEnvironment.get().equals(RegistryEnvironment.UNITTEST)
            || RegistryToolEnvironment.get() != null,
        "setJpamTm() should only be called by tools and tests.");
    readOnlyJpaTm = Suppliers.memoize(readOnlyJpaTmSupplier::get);
  }

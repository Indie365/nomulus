...
public class VKey<T> extends ImmutableObject implements Serializable {
  ...
  public String stringify() {
    ...
    String key = CLASS_TYPE + KV_SEPARATOR + this.getKind().getName();
    if (maybeGetSqlKey().isPresent()) {
      key += DELIMITER + SQL_LOOKUP_KEY + KV_SEPARATOR + SerializeUtils.stringify(this.getSqlKey());
    }
    if (maybeGetOfyKey().isPresent()) {
      key += DELIMITER + OFY_LOOKUP_KEY + KV_SEPARATOR + this.getOfyKey().getString();
    }
    return key;
  }

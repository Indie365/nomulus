                  ...
                  () -> {
                    ...
                    CriteriaQueryBuilder<DomainBase> queryBuilder =
                        CriteriaQueryBuilder.create(DomainBase.class)
                            ...
                            .orderBy(criteriaBuilder::asc, "fullyQualifiedDomainName");
                    if (cursorString.isPresent()) {
                      queryBuilder =
                          queryBuilder.where(
                    ...
                    if (partialStringQuery.getSuffix() != null) {
                      queryBuilder =
                          queryBuilder.where(
                              criteriaBuilder::equal, "tld", partialStringQuery.getSuffix());
                    }
                  ...
                  () -> {
                    CriteriaQueryBuilder<DomainBase> builder =
                        queryItemsSql(
                            ...
                            .orderBy(
                                jpaTm().getEntityManager().getCriteriaBuilder()::asc,
                                "fullyQualifiedDomainName");
          ...
          jpaTm()
              .transact(
                  () -> {
                    ...
                    //
                    // Note: there is no way to compare the native PSQL timestamp object directly
                    // here, so we just use the END_OF_TIME year.
                          ...
                          jpaTm()
                              ...
                              .createNativeQuery(
                                  "SELECT h.repo_id FROM \"Host\" h WHERE :address = "
                                      ...
                                      + "CAST(h.deletion_time AS text) LIKE :endOfTime")
                    ...
                    Stream<String> resultStream =
                        query
                            ...
                            .setParameter("endOfTime", String.format("%d%%", END_OF_TIME.getYear()))
                ...
                () -> {
                  for (VKey<HostResource> hostKey : hostKeys) {
                    ...
                    if (!shouldIncludeDeleted()) {
                      queryBuilder =
                          queryBuilder.where(
                              jpaTm().getEntityManager().getCriteriaBuilder()::greaterThan,
                              "deletionTime",
                              getRequestTime());
                    }
                    if (cursorString.isPresent()) {

...
public final class UpdateRegistrarRdapBaseUrlsAction implements Runnable {
  ...
  public void run() {
    ...
    Registrar.loadAllKeysCached()
        .forEach((key) -> tm().transact(() -> processRegistrar(key, ianaIdsToUrls)));
  }
  ...
  private void processRegistrar(
      VKey<Registrar> registrarKey, ImmutableMap<String, String> ianaIdsToUrls) {
    Registrar registrar = tm().loadByKey(registrarKey);
    // This shouldn't be possible, just in case the registrar was deleted
    if (registrar == null) {
      return;
    }
    ...
    ImmutableSet<String> baseUrls = baseUrl == null ? ImmutableSet.of() : ImmutableSet.of(baseUrl);
    if (registrar.getRdapBaseUrls().equals(baseUrls)) {
      logger.atInfo().log(
          "No change in RDAP base URLs for registrar %s", registrar.getRegistrarId());
      return;
    } else if (baseUrls.isEmpty()) {
      logger.atInfo().log("Removing RDAP base URLs for registrar %s", registrar.getRegistrarId());
    } else {
      logger.atInfo().log(

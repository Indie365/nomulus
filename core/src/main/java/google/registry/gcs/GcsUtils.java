...
public class GcsUtils implements Serializable {
  ...
  @Nullable private final GoogleCredentials credentials;

  private Storage storage() {
    return credentials == null
        ? STORAGE_OPTIONS.getService()
        : STORAGE_OPTIONS.toBuilder().setCredentials(credentials).build().getService();
  }

  @Inject
  public GcsUtils(@Nullable @DefaultCredential GoogleCredentialsBundle credentialsBundle) {
    credentials = credentialsBundle == null ? null : credentialsBundle.getGoogleCredentials();
  }
  ...
  private static BlobInfo createBlobInfo(BlobId blobId) {
    BlobInfo.Builder builder = BlobInfo.newBuilder(blobId).setContentType("no-cache");
    MediaType mediaType = EXTENSIONS.get(getLast(Splitter.on('.').split(blobId.getName())));
    if (mediaType != null) {
      builder = builder.setContentType(mediaType.type());
    }

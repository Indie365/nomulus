...
public final class DomainCreateFlow implements TransactionalFlow {
  ...
  private void verifyIsGaOrSpecialCase(
    ...
    // We allow all registration during GA.
    ...
    if (behavior.equals(RegistrationBehavior.BYPASS_TLD_STATE)
      ...
      // Trademarked and non-trademarked names get treated differently in sunrise or quiet periods
      boolean isTrademarkedName = claimsList.getClaimKey(domainLabel).isPresent();
      // Non-trademarked names with the state check bypassed are always available
      if (!isTrademarkedName) {
        return;
      } else if (currentState.equals(QUIET_PERIOD)
          && registry.getTldStateTransitions().headMap(now).containsValue(START_DATE_SUNRISE)) {
        // Allow trademarked names in post-sunrise quiet period if using an allocation token that
        // bypasses TLD state checks
        return;
      }
    }
    ...
    // Anchor tenant overrides all other considerations to allow registration
    if (isAnchorTenant) {
      return;
    }

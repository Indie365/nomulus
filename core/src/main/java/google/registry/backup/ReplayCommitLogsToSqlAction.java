...
public class ReplayCommitLogsToSqlAction implements Runnable {
    ...
    try {
      ...
      if (dryRun) {
        ...
        String dryRunMessage =
            String.format(
                "Running in dry-run mode, would have processed %d file(s).", commitLogFiles.size());
        logger.atInfo().log(dryRunMessage);
        response.setPayload(dryRunMessage);
        commitLogFiles.forEach(file -> logger.atInfo().log(file.getFilename().getObjectName()));
      } else {
  ...
  private ImmutableList<GcsFileMetadata> getFilesToReplay() {
    ...
    return diffLister.listDiffFiles(fromTime, /* current time */ null);
  }

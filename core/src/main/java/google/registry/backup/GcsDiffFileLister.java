...
class GcsDiffFileLister {
  ...
  ImmutableList<BlobInfo> listDiffFiles(DateTime fromTime, @Nullable DateTime toTime) {
    ...
    try {
      filenames = gcsUtils.listFolderObjects(gcsBucket, DIFF_FILE_PREFIX);
    } catch (IOException e) {
    ...
    for (String filename : filenames) {
      ...
      if (isInRange(upperBoundTime, fromTime, toTime)) {
        upperBoundTimesToBlobInfo.put(upperBoundTime, executor.submit(() -> getBlobInfo(filename)));
    ...
    boolean inconsistentFileSet =
        !constructDiffSequence(upperBoundTimesToBlobInfo, fromTime, lastUpperBoundTime, sequence);
    ...
    while (checkForMoreExtraDiffs) {
      ...
      for (DateTime key : upperBoundTimesToBlobInfo.descendingKeySet()) {
        ...
        if (!sequence.containsKey(key)) {
          constructDiffSequence(upperBoundTimesToBlobInfo, fromTime, key, sequence);

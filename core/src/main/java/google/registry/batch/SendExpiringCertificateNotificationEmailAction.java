...
public class SendExpiringCertificateNotificationEmailAction implements Runnable {
   ...
   */
  protected DateTime sendNotificationEmail(
      Registrar registrar,
        ...
        if (!recipients.isEmpty()) {
          sendEmailService.sendEmail(
              EmailMessage.newBuilder()
                  ...
                  .build());
          return DateTime.now(UTC).minusMinutes((int) UPDATE_TIME_OFFSET.getStandardMinutes());
        } else {
  ...
  /** Update the last notification sent date in database. */
  protected void updateLastNotificationSentDate(
     ...
     */
    try {
      if (!lastNotificationSentDate.equals(START_OF_TIME)) {
        tm().transact(
                () -> {
      ...
      logger.atWarning().withCause(e).log(
          "Failed to update the last notification sent date to Registrar %s "
              + "for the %s certificate .",
  ...
  protected void sendNotificationEmails() {
    for (Registrar registrar : getRegistrarsWithExpiringCertificates()) {
      DateTime lastNotificationSentDate =
      ...
      if (!lastNotificationSentDate.equals(START_OF_TIME)) {
        updateLastNotificationSentDate(
            registrar, lastNotificationSentDate, CertificateType.PRIMARY);
      }
      DateTime lastFailOverNotificationSentDate =
          sendNotificationEmail(
              registrar,
              registrar.getLastExpiringFailoverCertNotificationSentDate(),
              CertificateType.FAILOVER,
              registrar.getFailoverClientCertificate());
      if (!lastFailOverNotificationSentDate.equals(START_OF_TIME)) {
        updateLastNotificationSentDate(
            registrar, lastFailOverNotificationSentDate, CertificateType.PRIMARY);
      }
    }
    logger.atInfo().log(
        "Notification Emails have been sent to all registrars that contain "
   ...
   * parameters.
   */
  protected String getEmailBody(String registrarName, CertificateType type, Date expirationDate) {
    if (expirationDate == null || type == null) {
      throw new NullPointerException("The argument(s) cannot be null.");
    }

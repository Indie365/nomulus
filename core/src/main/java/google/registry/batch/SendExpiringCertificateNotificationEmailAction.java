...
public class SendExpiringCertificateNotificationEmailAction implements Runnable {
   ...
   * used to handle corner cases when the update happens in between the day switch. For instance,if
   * the job starts at 2:00 am every day and it finishes at 2:03 of the same day, then next day at
   * 2am, the date difference will be less than a day, which will lead to the date difference
   * between two successive email sent date being the expected email interval days + 1;
   */
    ...
    for (Registrar registrar : registrars) {
      ...
      try {
        ...
        if (!recipients.isEmpty()) {
          ...
          // certificate(s)
          DateTime lastNotificationSentDate = DateTime.now(UTC).minusMinutes(UPDATE_TIME_OFFSET);
  ...
  /** Generate email content that takes registrar name and expiration date as parameters */

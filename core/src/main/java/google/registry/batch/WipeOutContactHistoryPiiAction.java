...
public class WipeOutContactHistoryPiiAction implements Runnable {
  ...
  @VisibleForTesting
  int wipeOutContactHistoryData(ScrollableResults oldContactHistoryData) {
    int numOfEntities = 0;
    ImmutableList.Builder<ContactHistory> contactHistoryBatchBuilder =
        new ImmutableList.Builder<>();
    while (oldContactHistoryData.next()) {
      contactHistoryBatchBuilder.add((ContactHistory) oldContactHistoryData.get(0));
      numOfEntities++;
      if (numOfEntities != 0 && (numOfEntities % wipeOutQueryBatchSize == 0)) {

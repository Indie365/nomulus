...
public class DeleteProberDataAction implements Runnable {
  ...
  public void run() {
    ...
    if (tm().isOfy()) {
      ...
      ImmutableList<DomainBase> relevantDomains = loadRelevantDomainsSql(deletableTlds);
      Lists.partition(relevantDomains, SQL_BATCH_SIZE).forEach(this::processDomainBatch);
    }
  }

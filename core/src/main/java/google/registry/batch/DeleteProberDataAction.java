...
public class DeleteProberDataAction implements Runnable {
  ...
  private void runSqlJob(ImmutableSet<String> deletableTlds) {
    ...
    do {
      // Lambda function requires a "final" variable
      final String previousLastSeenRepoId = lastSeenRepoId;
      lastSeenRepoId =
          jpaTm()
              .transactWithoutBackup(
                  () -> {
                    ImmutableList<DomainBase> nextBatch =
                        nextDomainBatch(deletableTlds, previousLastSeenRepoId);
                    processDomainBatch(nextBatch, softDeletedDomains, hardDeletedDomains);
                    return nextBatch.isEmpty() ? null : Iterables.getLast(nextBatch).getRepoId();
                  });
  ...
  private void processDomainBatch(
      ImmutableList<DomainBase> domains,

...
public class DeleteProberDataAction implements Runnable {
  ...
  private ImmutableList<DomainBase> loadRelevantDomainsSql(
      ImmutableSet<String> deletableTlds, ImmutableSet<String> proberRoidSuffixes) {
    DateTime now = tm().getTransactionTime();
    ...
    // 5. Either be active (creationTime <= now < deletionTime) or have been deleted a while ago
    // (this prevents accidental double-map with the same key from immediately deleting active
    // domains)
    // 6. Have a repo ID ending in one of the specified ROID suffixes
    return jpaTm()
        .transact(
            () ->
                jpaTm()
                    .query(
                        "FROM Domain d WHERE d.tld IN :tlds AND d.fullyQualifiedDomainName NOT"
                            + " LIKE 'nic.%' AND (d.subordinateHosts IS EMPTY OR"
                            + " d.subordinateHosts IS NULL) AND d.creationTime <"
                            + " :creationTimeCutoff AND ((d.creationTime <= :nowAutoTimestamp AND"
                            + " d.deletionTime > current_timestamp()) OR d.deletionTime <"
                    ...
                    .filter(
                        domain ->
                            proberRoidSuffixes.contains(
                                Iterables.getLast(Splitter.on('-').split(domain.getRepoId()))))
                    .collect(toImmutableList()));
  }

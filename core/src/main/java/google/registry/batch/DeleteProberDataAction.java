...
public class DeleteProberDataAction implements Runnable {
  ...
  String registrAdminRegistrarId;
  ...
  public void run() {
    checkState(
        !Strings.isNullOrEmpty(registrAdminRegistrarId),
    ...
    if (tm().isOfy()) {
      mrRunner
          ...
          .runMapOnly(
              new DeleteProberDataMapper(proberRoidSuffixes, isDryRun, registrAdminRegistrarId),
              ImmutableList.of(EppResourceInputs.createKeyInput(DomainBase.class)))
          .sendLinkToMapreduceConsole(response);
    } else {
      tm().transact(() -> deleteProberDataSql(proberRoidSuffixes, deletableTlds));
    }
  }

  private void deleteProberDataSql(
      ImmutableSet<String> proberRoidSuffixes, ImmutableSet<String> deletableTlds) {
    loadRelevantDomainsSql(deletableTlds)
        .forEach(domain -> deleteDomain(domain, proberRoidSuffixes));
  }

  private void deleteDomain(DomainBase domain, ImmutableSet<String> proberRoidSuffixes) {
    ...
    if (EppResourceUtils.isActive(domain, now)) {
      if (isDryRun) {
        ...
        softDeleteDomain(domain, registrAdminRegistrarId, dnsQueue);
      }
      return;
    }
  ...
  private Stream<DomainBase> loadRelevantDomainsSql(ImmutableSet<String> deletableTlds) {
    return jpaTm()
        .query(
        ...
        .getResultStream()
        .filter(domain -> domain.getSubordinateHosts().isEmpty());
  }

 ...
 *
 * <p>This command only operates over domains where the most recent history object is synthetic,
 * occurred after the bad pipeline's end time, and occurred before we've begun running this
 * pipeline. That way, this should fix any bad dsData entries created before while remaining
...
public class CreateSyntheticDomainHistoriesCommand extends ConfirmingCommand
  ...
  private ImmutableSet<VKey<Domain>> keys;
            ...
            () -> {
              keys =
                  jpaTm()
                      ...
                      .getResultStream()
                      .map(r -> VKey.create(Domain.class, r))
                      .collect(toImmutableSet());
            });
    return String.format(
        "Attempt to create synthetic history entries for %d domains?", keys.size());
  }

  @Override
  protected String execute() throws Exception {
    List<Future<?>> futures = new ArrayList<>();
    for (VKey<Domain> key : keys) {
      futures.add(
          executor.submit(
              () -> {

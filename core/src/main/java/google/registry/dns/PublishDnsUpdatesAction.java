...
public final class PublishDnsUpdatesAction implements Runnable, Callable<Void> {
  ...
  private final String dnsUpdateFailRegistrySupportEmail;
  ...
  public PublishDnsUpdatesAction(
      ...
      @Config("dnsUpdateFailRegistrySupportEmail") String dnsUpdateFailRegistrySupportEmail,
    ...
    this.dnsUpdateFailRegistrySupportEmail = dnsUpdateFailRegistrySupportEmail;
  }
    ...
    try {
      ...
      if (domains.size() > 1 || hosts.size() > 1) {
        ...
        domains.forEach(
            dn -> {
              Domain domain =
                  loadByForeignKey(Domain.class, dn, clock.nowUtc())
                      .orElseThrow(
                          () ->
                              new NotFoundException(
                                  String.format("Domain entity for %s not found", dn)));

              notifyWithEmailAboutDnsUpdateFailure(
                  domain.getCurrentSponsorRegistrarId(), dn, false);
            });

        hosts.forEach(
            hn -> {
              Host host =
                  loadByForeignKey(Host.class, hn, clock.nowUtc())
                      .orElseThrow(
                          () ->
                              new NotFoundException(
                                  String.format("Host entity for %s not found", hn)));

              notifyWithEmailAboutDnsUpdateFailure(
                  host.getPersistedCurrentSponsorRegistrarId(), hn, true);
            });
      }
  ...
  private void notifyWithEmailAboutDnsUpdateFailure(
    ...
    sendEmailUtils.sendEmail(
        ...
        String.format(
            ...
            hostOrDomainName,
            isHost ? "host" : "domain",
            dnsUpdateFailRegistrySupportEmail,

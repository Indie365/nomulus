...
public final class IcannReportingUploadAction implements Runnable {
  ...
  private void uploadReport(
    ...
    tm().transact(
            () -> {
              // In order to keep the transactions short-lived, we load all of the cursors in a
              // single transaction then later use one transaction per cursor when uploading the
              // data / updating the cursors. We run behind a lock so the cursors shouldn't be
              // changed, but double check to be sure.

...
public abstract class HtmlAction implements Runnable {
  ...
  public void run() {
    ...
    String username;
    String email;
    if (authInfo.user().isPresent()) {
      username = authInfo.user().get().getNickname();
      email = authInfo.user().get().getEmail();
    } else {
      username = authInfo.consoleUser().get().getEmailAddress();
      email = username;
    }
    ...
    data.put("xsrfToken", xsrfTokenManager.generateToken(email));

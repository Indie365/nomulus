...
public class RegistrarSettingsAction implements Runnable, JsonActionRunner.JsonAction {
  ...
  private RegistrarResult update(final Map<String, ?> args, String registrarId) {
    EmailInfo emailInfo = tm().transact(() -> saveUpdates(args, registrarId));
    // Email the updates
    if (sendExternalUpdatesIfNecessary(
                emailInfo.registrar(),
                emailInfo.contacts(),
                emailInfo.updatedRegistrar(),
                emailInfo.updatedContacts())
            == true
        && isInTestDriver.get() == false) {
      // Enqueues a sync registrar sheet task only if the update email is sent successfully and
      // enqueuing is not triggered by console tests
   ...
   */
  private Boolean sendExternalUpdatesIfNecessary(
    ...
    return sendEmailUtils.sendEmail(
        String.format(

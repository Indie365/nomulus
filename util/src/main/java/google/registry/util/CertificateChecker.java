...
public class CertificateChecker {
   ...
   *
   * <p>The max validity length schedule is a sorted map consisting of DateTime to Integer entries
   * representing a maximum validity period for certificates issued on or after that time. The first
   * entry must have a key of {@link DateTimeUtils#START_OF_TIME}, such that every possible date has
   * an applicable max validity period. Since security requirements tighten over time, the max
   * validity periods will be decreasing as the date increases.
   *
   ...
   * to this method with a call to {@link #checkCertificate(X509Certificate) to determine other
   * issues with the certificate that may be occurring.
   */
